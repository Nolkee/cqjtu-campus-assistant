...
String _redactUsernameForLog(String username) {
...
String _sanitizeSensitiveText(String text) {
  var sanitized = text;
  const keys = [
    'password',
    'ticket',
...
class SessionManager {
  ...
  Future<void> restoreLoginState(String username, String sessionId) async {
    ...
    if (casCookies != null && casCookies.isNotEmpty) {
      debugPrint(
        '[SessionManager] restoreLoginState inject CAS cookies username=${_redactUsernameForLog(username)}',
      );
    ...
    if (jwgCookies != null && jwgCookies.isNotEmpty) {
      debugPrint(
        '[SessionManager] restoreLoginState inject JWG cookies username=${_redactUsernameForLog(username)}',
      );
      await _api.injectCookies(
        username,
        _jwgDomain,
    ...
    if (ecardCookies != null && ecardCookies.isNotEmpty) {
      debugPrint(
        '[SessionManager] restoreLoginState inject ECARD cookies username=${_redactUsernameForLog(username)}',
      );
      await _api.injectCookies(
        username,
        _ecardDomain,
    ...
    try {
      ...
      if (!recoverable) {
        await _markFailure(
          ...
          username: username,
          failureKind: kind,
          message: _sanitizeSensitiveText(error.toString()),
          retryCount: 0,
          latency: stopwatch.elapsed,
        );
        rethrow;
      ...
      try {
        ...
        await _markFailure(
          ...
          message: _sanitizeSensitiveText(retryError.toString()),
          retryCount: 1,
          latency: stopwatch.elapsed,
        );
